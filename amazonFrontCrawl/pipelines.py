...
from items import amazon_product_review_percent_info, amazon_product_review_result, amazon_top_reviewers,TodayDealItem,AdvertItem
...
class AdvertPipeline():
    ...
    def process_item(self, item, spider):
        if isinstance(item, AdvertItem):
            query = self.dbpool.runInteraction(self._advert_insert, item)  # 调用插入的方法
            query.addErrback(self._handle_error, item, spider)  # 调用异常处理方法
        else:
            pass
        return item
    ...
    # feedback 写入数据库中
    def _advert_insert(self, tx, item):
        # print item['name']
        dt = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        sql = r'insert into advert (date,zone,keyword,position,page,' \
              r'serial_num,asin,is_sponsored,create_time,update_time) values("%s","%s","%s","%s",%d,%d,"%s",%d,"%s","%s");'%(item["date"],
                    item["zone"], item["keyword"], item["position"], item["page"], item["serial_num"],
                  item["asin"], item["is_sponsored"], dt, dt)
        print("insert data to monitor_advert ......%s"%item["keyword"])
        logging.info("insert data to monitor_advert ......")
        tx.execute(sql)
        print("insert data into mLonitor_advert success ......")
